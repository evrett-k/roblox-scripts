        ...
        for _, player in pairs(Players:GetPlayers()) do
            ...
            if char and root and hum and head and hum.Health > 0 then
                if not existing then
                    ...
                    -- Update TextLabel and Distance dynamically
                    ...
                    -- Dynamically update team colors if they switch teams mid-game
                ...
                -- Cleanup ESP folder immediately if the player dies or character unloads
...
TabMove:AddSlider({
    ...
    Increment = 1,local OrionLib = loadstring(game:HttpGet(('https://raw.githubusercontent.com/jensonhirst/Orion/main/source')))()
local Window = OrionLib:MakeWindow({
    Name = "Mod Menu",
    HidePremium = true,
    SaveConfig = true,
...
local function toggleNoclip(state)
    ...
    if state then
        ...
        -- Force collisions back on instantly
...
TabBinds:AddBind({
    ...
    Callback = function()
        pcall(function() 
            noclipUIToggle:Set(not noclipEnabled) 
        end)
    end
})
